...
class Node {
	...
	addSubString(str) {
		...
		if (!this.children[str.at(0)]) {
			const child = new Node(str.at(0));
			this.children[child.value] = child;
			child.addSubString(str.slice(1));
		} else {
			this.children[str.at(0)].addSubString(str.slice(1));
		}
	}
	getDescendantStrings() {
		const childrenArray = Object.values(this.children);
		if (!childrenArray.length) return [this.value];

		let descendantStrings = [];
		for (const childNode of childrenArray) {
			descendantStrings = descendantStrings.concat(
				childNode.getDescendantStrings()
			);
		}
		return descendantStrings.map((subStr) => this.value + subStr);
	}
	complete(string) {
		string = string.toLowerCase();
		if (!string.length || string === this.value)
			return this.getDescendantStrings();
		if (this.value && string.at(0) !== this.value) return [];

		const charIdx = this.value ? 1 : 0; // If root node, delegate comparisons to child
		const nextNode = this.children[string.at(charIdx)];
		return nextNode
			? nextNode
					.complete(string.slice(charIdx))
					.map((subStr) => this.value + subStr)
			: [];
	}
}
...
describe("tries", function () {
	...
	test("dataset of 925 – san", () => {
		...
		const completions = root.complete("santa c");
		expect(completions.length).toBe(3);
		expect(
			_.intersection(completions, [
				"san antonio",
